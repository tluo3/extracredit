...
let s:figure =     "\\begin{figure}<+[htpb]+>\<cr>\\begin{center}\<cr>\\includegraphics[width=<+\\textwidth+>]{<++>}\<cr>\\end{center}\<cr>\\caption{<+caption text+>}\<cr>\\label{fig:<+label+>}\<cr>\\end{figure}<++>"
